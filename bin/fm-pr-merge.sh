...
# The Gerrit adapter is read-only, so there is no merge path here to reach and
# the refusal is stated rather than left as a silently absent provider branch.
...
if [ "$PROVIDER" = gerrit ]; then
  echo "error: firstmate does not submit a Gerrit change: the Gerrit adapter is read-only, because submitting requires an attributed human approval it must not manufacture" >&2
  exit 2
fi
shift 2
