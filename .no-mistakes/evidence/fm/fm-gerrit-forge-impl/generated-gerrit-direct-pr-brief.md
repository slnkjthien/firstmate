...
# Setup
You are in a disposable git worktree of ger, at a detached HEAD on a clean default branch.
...
1. First action: create your branch: `git checkout -b fm/gtask`
...
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/fm-gerrit-live.EvyI/home/state/gtask.status' && { [ ! -e '/tmp/fm-gerrit-live.EvyI/home/config/fleet-ledger' ] || '/tmp/fm-gerrit-live.EvyI/home/bin/fm-fleet-ledger.sh' appended '/tmp/fm-gerrit-live.EvyI/home/config' '/tmp/fm-gerrit-live.EvyI/home/state/gtask.status' >/dev/null 2>&1 || true; }`
...
Firstmate steers you through durable message files in '/tmp/fm-gerrit-live.EvyI/home/state/gtask.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-gerrit-live.EvyI/home/state/gtask.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-gerrit-live.EvyI/home/state/gtask.inbox'/NNN.msg '/tmp/fm-gerrit-live.EvyI/home/state/gtask.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/tmp/fm-gerrit-live.EvyI/home/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/tmp/fm-gerrit-live.EvyI/home/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
