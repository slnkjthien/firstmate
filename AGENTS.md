...
Delivery mode and `yolo` are orthogonal, and a project's registered forge is orthogonal to both.
...
On a `forge=gerrit` project there is no PR at all: the worker stops at `done [at=<epoch>]: ready in branch fm/<id> - no-mistakes passed, fixes recovered`, and publishing that branch is firstmate's step, never the worker's.
